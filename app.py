...
with tab1:
    ...
    def cost_waterfall_figure(df_in: pd.DataFrame, unit_mode_local: str) -> go.Figure:
        ...
        fig.update_layout(
            title=f"2026 Cost Bridge: {_latest_actual_name} Monthly Run-Rate → Dec Forecast Run-Rate",
    ...
    def hc_waterfall_figure(df_in: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            title=f"2026 HC Bridge: {_latest_actual_name} Closing HC to Dec Forecast",
            title_font=dict(family=plotly_font(), size=18, color="#1f2937"),
            plot_bgcolor="white",
            paper_bgcolor="white",
            font=dict(family=plotly_font(), color="#374151", size=12),
...
def compute_scenario_impacts(df_in: pd.DataFrame, levers: dict) -> dict:
    ...
    # --- Solver Lever: evenly-spread exits/hires (independent of Lever 4) ---
    solver_applied = levers.get("solver_applied", None)
    if solver_applied:
        s_grade     = solver_applied.get("grade", "")
        s_n_month   = int(solver_applied.get("n_per_month", 0))
        s_start_num = int(solver_applied.get("start_month_num", 4))
        s_rate      = float(solver_applied.get("rate", 0))
        s_verb      = solver_applied.get("verb", "exit")
        if s_n_month > 0 and s_rate > 0:
            s_n_months = 13 - s_start_num
            s_triangle = s_n_months * (s_n_months + 1) / 2
            s_saving   = s_n_month * s_rate * s_triangle
            s_total_hc = s_n_month * s_n_months
            # exits reduce cost (negative delta), hires increase cost (positive delta)
            delta_cost = -s_saving if s_verb == "exit" else +s_saving
            delta_hc   = -s_total_hc if s_verb == "exit" else +s_total_hc
            lever_rows.append({
                "Lever": f"Solver: {s_verb} {s_n_month} {s_grade}/month from {MONTHS[s_start_num-1]} (evenly spread)",
                "Delta HC":   int(delta_hc),
                "Delta Cost": delta_cost,
            })
            total_cost_delta += delta_cost
            total_hc_delta   += int(delta_hc)
...
with tab2:
    ...
    # ── AFFORDABILITY SOLVER ─────────────────────────
    # Pre-compute forecast df needed by both solver and lever panel
    ...
    with st.expander("Open Affordability Solver", expanded=False):
        ...
        # Scope note
        scope_note = " | ".join([s for s in [
            f"MT: {selected_mt}", f"Country: {selected_country}"
        ...
        st.markdown(
            ...
            f"⚙️ <strong>Scope:</strong> {scope_note} — exits/hires distributed "
            f"proportionately across countries within this scope.</div>",
            unsafe_allow_html=True
        )

        # ── Row 1: Target + Strategy side by side ──
        r1c1, r1c2 = st.columns([1, 1], gap="large")

        with r1c1:
            st.markdown("<div style='font-size:11px;font-weight:700;color:#5f6878;"
                        "letter-spacing:0.05em;margin-bottom:6px;'>COST TARGET</div>",
                        unsafe_allow_html=True)
            target_mode = st.radio(
                "Target basis", ["Meet CP26 budget", "Custom target"],
                horizontal=True, key="sol_target_mode"
            )
            if target_mode == "Custom target":
                custom_target_val = st.number_input(
                    f"Custom target ({unit_mode})",
                    value=round(cp26_budget_cost / get_scale(unit_mode), 1),
                    step=0.1 if unit_mode == "$m" else 100.0,
                    key="sol_custom_target"
                )
                target_cost = custom_target_val * get_scale(unit_mode)
            else:
                target_cost = cp26_budget_cost

            required_saving = current_wfp_outlook_cost - target_cost
            is_overrun = required_saving > 0
            gap_colour = "#b91c1c" if is_overrun else "#15803d"
            gap_bg     = "rgba(239,68,68,0.08)" if is_overrun else "rgba(34,197,94,0.08)"
            gap_label  = "Overrun to close" if is_overrun else "Headroom available"
            verb       = "exit" if is_overrun else "hire"

            st.markdown(
                f"<div style='background:{gap_bg};border-radius:10px;padding:10px 14px;margin-top:6px;"
                f"display:flex;align-items:center;gap:16px;'>"
                f"<div><div style='font-size:10px;color:{gap_colour};font-weight:700;"
                f"text-transform:uppercase;letter-spacing:0.05em;'>{gap_label}</div>"
                f"<div style='font-size:22px;font-weight:700;color:{gap_colour};'>"
                f"{fmt_money(abs(required_saving), unit_mode)}</div></div>"
                f"<div style='font-size:11px;color:{gap_colour};'>"
                f"{'↑ Heads to exit' if is_overrun else '↓ Heads to hire'}</div>"
                f"</div>",
                unsafe_allow_html=True
            )
        ...
        with r1c2:
            st.markdown("<div style='font-size:11px;font-weight:700;color:#5f6878;"
                        "letter-spacing:0.05em;margin-bottom:6px;'>EXIT / HIRE STRATEGY</div>",
                        unsafe_allow_html=True)
            strategy_options = {
                "Evenly Apr–Dec (9 months)":     4,
                "Start Q3 — Jul–Dec (6 months)": 7,
                "Start Q4 — Oct–Dec (3 months)": 10,
            }
            strategy_label  = st.radio("Start month", list(strategy_options.keys()),
                                       key="sol_strategy", horizontal=False)
            start_month_num = strategy_options[strategy_label]
            n_months        = 13 - start_month_num
            triangle        = n_months * (n_months + 1) / 2

        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)

        # ── Row 2: Results table + Apply buttons + Dec HC ──
        fcast_for_solver = df_scope[df_scope["Month_Type"].str.lower() == "forecast"]
        grade_rates = {}
        _all_grades = sorted(df_scope["Grade"].dropna().unique().tolist())
        for g in _all_grades:
            g_rows = fcast_for_solver[fcast_for_solver["Grade"] == g]
            if not g_rows.empty:
                rate = float(g_rows["Monthly_Rate_USD"].mean())
                if rate > 0:
                    grade_rates[g] = rate

        # Weighted average rate (weighted by forecast HC at Dec)
        dec_fcast_solver = fcast_for_solver[fcast_for_solver["Month"] == "Dec"]
        total_wt_hc = dec_fcast_solver["Closing_HC"].sum()
        wtd_rate = (
            float((dec_fcast_solver["Closing_HC"] * dec_fcast_solver["Monthly_Rate_USD"]).sum() / total_wt_hc)
            if total_wt_hc > 0 else float(fcast_for_solver["Monthly_Rate_USD"].mean())
        )
        ...
        for grade, rate in sorted(grade_rates.items(), key=lambda x: -x[1]):
            if triangle > 0 and rate > 0:
                ...
                saving    = n_ceil * rate * triangle
                solver_rows.append({
                    "Grade":         grade,
                    "Monthly Rate":  fmt_money(rate, "$000s"),
                    f"{verb.title()} / Month": n_ceil,
                    "Total HC":      total_hc,
                    "Cost Impact":   fmt_money(saving, unit_mode),
                    "_rate": rate, "_n_ceil": n_ceil,
                    "_total": total_hc, "_start_num": start_month_num,
                })

        # Weighted average row
        if triangle > 0 and wtd_rate > 0:
            n_wtd  = abs(required_saving) / (wtd_rate * triangle)
            n_wtd_ceil = max(1, int(n_wtd + 0.9999))
            total_wtd  = n_wtd_ceil * n_months
            saving_wtd = n_wtd_ceil * wtd_rate * triangle
            solver_rows.append({
                "Grade":         "Weighted Avg",
                "Monthly Rate":  fmt_money(wtd_rate, "$000s"),
                f"{verb.title()} / Month": n_wtd_ceil,
                "Total HC":      total_wtd,
                "Cost Impact":   fmt_money(saving_wtd, unit_mode),
                "_rate": wtd_rate, "_n_ceil": n_wtd_ceil,
                "_total": total_wtd, "_start_num": start_month_num,
            })

        if solver_rows:
            dec_hc_now = df_scope[df_scope["Month"].eq("Dec")]["Closing_HC"].sum()

            # Build display table with Dec HC column
            display_rows = []
            for row in solver_rows:
                dec_after = dec_hc_now - row["_total"]
                vs_bud    = dec_after - cp26_budget_hc
                display_rows.append({
                    "Grade":         row["Grade"],
                    "Monthly Rate":  row["Monthly Rate"],
                    f"{verb.title()} / Month": row[f"{verb.title()} / Month"],
                    "Total HC":      row["Total HC"],
                    "Cost Impact":   row["Cost Impact"],
                    f"Dec HC after {verb}": int(round(dec_after)),
                    "vs Budget":     f"{'▼' if vs_bud<0 else '▲'} {fmt_num(abs(vs_bud))}",
                    "_vs_bud":       vs_bud,
                    "_is_wtd":       row["Grade"] == "Weighted Avg",
                })

            def style_solver_row(row):
                if row.get("_is_wtd", False):
                    return ["background-color:#e8edf5;color:#04145f;font-weight:700;"] * len(row)
                return [""] * len(row)

            def colour_vs_budget(val):
            ...
            display_cols = ["Grade", "Monthly Rate", f"{verb.title()} / Month",
                            "Total HC", "Cost Impact", f"Dec HC after {verb}", "vs Budget"]

            df_display_clean = df_display[display_cols].copy()

            styled_sol = (
                df_display_clean.style
                .apply(lambda row: (
                    ["background-color:#e8edf5;color:#04145f;font-weight:700;"] * len(row)
                    if str(df_display.loc[row.name, "_is_wtd"]) == "True"
                    else [""] * len(row)
                ), axis=1)
                .map(colour_vs_budget, subset=["vs Budget"])
                .format({"Total HC": "{:,}", f"Dec HC after {verb}": "{:,}"})
                .set_table_styles([
                    {"selector": "thead th", "props": [
                        ("background-color","#04145f"),("color","white"),
            ...
            st.markdown(
                f"<div style='font-size:12px;color:#1f2937;margin-bottom:8px;'>"
                f"Heads to <strong>{verb}</strong> per month "
                f"(<strong>{MONTHS[start_month_num-1]}–Dec, {n_months} months</strong>) "
                f"to close the <strong>{fmt_money(abs(required_saving), unit_mode)}</strong> gap. "
                f"Bottom row (shaded) = grade-mix weighted average across this scope.</div>",
                unsafe_allow_html=True
            )
            st.write(styled_sol.to_html(), unsafe_allow_html=True)

            st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
            st.markdown(
                "<div style='font-size:11px;color:#5f6878;background:#f9fafb;border-radius:8px;"
                "padding:8px 12px;margin-bottom:8px;'>"
                "ℹ️ Select one grade option and click Apply. The solver uses the <strong>evenly-spread model</strong> "
                "(n heads exiting each month) which feeds directly into the Scenario Outlook — "
                "independent of Lever 4. Only one solver option can be active at a time.</div>",
                unsafe_allow_html=True
            )

            # Apply buttons — visible text on white background
            st.markdown(
                "<div style='font-size:11px;font-weight:700;color:#5f6878;"
                "letter-spacing:0.05em;margin-bottom:8px;'>APPLY TO SCENARIO (evenly spread, independent of Lever 4):</div>",
                unsafe_allow_html=True
            )

            # Show active solver lever status
            active_solver = st.session_state.get("solver_applied", None)
            if active_solver:
                st.markdown(
                    f"<div style='background:rgba(19,105,226,0.10);border:1px solid #1369e2;"
                    f"border-radius:8px;padding:8px 12px;margin-bottom:10px;font-size:12px;color:#04145f;'>"
                    f"<strong>✓ Solver active:</strong> {active_solver['verb']} "
                    f"{active_solver['n_per_month']} {active_solver['grade']}/month "
                    f"from {MONTHS[active_solver['start_month_num']-1]} "
                    f"(evenly spread) — reflected in Scenario Outlook</div>",
                    unsafe_allow_html=True
                )

            # Radio to select ONE grade option, then single Apply button
            grade_labels = [r["Grade"] for r in solver_rows]
            grade_display = {
                r["Grade"]: f"{r['Grade']} — {r['Exit / Month'] if 'Exit / Month' in r else r.get('Hire / Month', r['_n_ceil'])}/mo · {r['Total HC']} total · saving {r['Cost Impact']}"
                for r in solver_rows
            }
            st.markdown(
                "<div style='font-size:11px;font-weight:700;color:#5f6878;"
                "letter-spacing:0.05em;margin-bottom:6px;'>SELECT GRADE OPTION TO APPLY:</div>",
                unsafe_allow_html=True
            )
            selected_grade_label = st.radio(
                "Grade option",
                options=grade_labels,
                format_func=lambda g: grade_display.get(g, g),
                key="sol_grade_select",
                label_visibility="collapsed"
            )
            selected_row = next((r for r in solver_rows if r["Grade"] == selected_grade_label), solver_rows[0])

            app_col, clr_col = st.columns([3, 1], gap="small")
            with app_col:
                with st.form(key="form_apply_solver"):
                    submitted = st.form_submit_button(
                        f"✓ Apply — {selected_grade_label} ({selected_row['_n_ceil']}/month from {MONTHS[start_month_num-1]})",
                        use_container_width=True,
                        type="primary"
                    )
                    if submitted:
                        is_wtd = selected_grade_label == "Weighted Avg"
                        st.session_state["solver_applied"] = {
                            "grade":           "All grades" if is_wtd else selected_grade_label,
                            "n_per_month":     selected_row["_n_ceil"],
                            "start_month_num": start_month_num,
                            "rate":            selected_row["_rate"],
                            "verb":            verb,
                        }
                        st.rerun()
            with clr_col:
    ...
    with lev_col:
        ...
        # _fcast_df_prev already defined above solver section
        ...
        accel_exits_hc = st.number_input(
            ...
            value=0,
            step=5,
            help="Heads exiting above current WFP plan on a specific date. For evenly-spread exits use the Affordability Solver above.",
            label_visibility="collapsed"
        )
        if accel_exits_hc > 0:
            ...
            with l4col2:
                accel_month_name = st.selectbox("From month", forecast_months_only,
                                                index=0, key="accel_month")
...
with tab3:
    ...
    st.markdown(
        "<p style='font-size:13px;color:#5f6878;margin-bottom:10px;'>"
        "% of forecast HC movements that are committed vs non-committed. "
        "High non-committed % = more flexibility to challenge. "
    ...
    comm_rows = []
    nc_hc_cols = [f"{p}_NonCommitted_HC" for _, p in POSITIVE_DRIVERS + NEGATIVE_DRIVERS]
    ...
    for mt in mts_ordered:
        ...
        if total > 0:
            comm_rows.append({
                ...
                "Non-Committed %": round(non_committed / total * 100, 1),
                "Committed HC":    int(round(committed)),
                "Non-Committed HC": int(round(non_committed)),
            })
    comm_df = pd.DataFrame(comm_rows).sort_values("Non-Committed %", ascending=False)
    ...
    fig_comm.add_bar(
        x=comm_df["MT"], y=comm_df["Non-Committed %"],
        name="Non-Committed (Challengeable)", marker_color="#f59e0b",
        hovertemplate="<b>%{x}</b><br>Non-Committed: %{y:.1f}%<extra></extra>"
    )
...
1. THE OUTLOOK IS THE FULL PLAN — COMMITTED AND NON-COMMITTED COMBINED:
   ...
   both committed (approved, in-progress) AND non-committed (planned but not yet approved).
   Non-committed growth is NOT a future risk to the outlook — it IS already IN the outlook.
   NEVER say "if non-committed growth materialises, outlook will worsen" — it already has.

2. WHAT NON-COMMITTED GROWTH ACTUALLY MEANS:
   Non-committed growth (shown as "Challengeable Growth" in the tool) represents the portion
   of the outlook that could still be blocked or delayed. It is a DOWNSIDE SAVING OPPORTUNITY,
   not an upside risk. If non-committed growth is $4m, that means:
...
3. COMMITTED VS NON-COMMITTED SPLIT:
   ...
   - The Scenario Builder allows users to block non-committed growth to model the saving potential.

4. OUTLOOK vs BUDGET:
   - If outlook < budget: position is favourable. Non-committed growth is buffer/headroom.
   - If outlook > budget: overrun exists. Non-committed growth is the first thing to challenge.
   - CRR Line of Sight = outlook - non-committed growth + non-committed reductions.
     This is the "do nothing challengeable" position — what happens if all non-committed
     growth is blocked AND all non-committed reductions land.
...
7. COUNTRY-LEVEL DETAIL:
   ...
   - Frame exits as a last resort — always mention challenging non-committed growth first
...
Apply the data model rules above strictly — especially rule 1 about non-committed growth.
